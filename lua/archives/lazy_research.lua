...
require("lazy").setup({
  ...
  -- VimTeX for LaTeX
  {
    ...
    ft = "tex",
    config = function()
      vim.g.vimtex_view_method = "zathura" -- Use Zathura (Linux) or change to "skim" (macOS)
      vim.g.vimtex_compiler_method = "latexmk"
      vim.g.vimtex_quickfix_mode = 2 -- Open quickfix on compile errors
  ...
  -- Which-key for keybinding hints
  {
    ...
    config = function()
      ...
      vim.keymap.set("n", "<leader>?", ":WhichKey<CR>", { desc = "Show Keybindings" })
    end,
  },
}, {
  performance = {
...
vim.keymap.set("n", "<leader>cv", ":VimtexView<CR>", { desc = "View LaTeX Output" })

-- Optional: Set up LSP for LaTeX and Python
...
lspconfig.pyright.setup({}) -- For Python research scripts
