-- example grepnav.nvim setup: 
--
-- require("lib_grepnav").setup({
--   engine_priority = { "ag" },  -- never fall back to plain grep
...
local config = {
  ...
  engine_priority = { "ag", "ack", "rg", "grep" },

  root_markers = {
    ".git", ".svn", ".hg", "Makefile", "package.json", "Cargo.toml",
    "go.mod", "pyproject.toml", "setup.py", "pom.xml", "build.gradle", "README.md",
  },
...
local function run_grep(pattern, root)
  ...
  elseif tool == "ack" then
    vim.list_extend(cmd, {
      "ack", "--with-filename", "--column", "--smart-case", "--follow"
    })
    vim.list_extend(cmd, ignore_flags)
    table.insert(cmd, pattern)
  ...
  for _, line in ipairs(output) do
    if not line:match("^Binary file") and not line:match("error:") then
