...
local function rename_current()
  if not (state.buf and state.win) then return end

  local lnum = vim.api.nvim_win_get_cursor(state.win)[1]
  local entry = vim.api.nvim_buf_get_lines(state.buf, lnum-1, lnum, false)[1]
  if not entry or entry == "../" then
    vim.notify("Cannot rename this entry", vim.log.levels.WARN)
    return
  end

  local full_old = get_full_path(entry)
  local base_old = entry:gsub("/$", "")

  vim.ui.input({ prompt = "Rename: ", default = base_old }, function(new_name)
    if not new_name or new_name == "" or new_name == base_old then return end

    local full_new = join_path(state.cwd, new_name)

    local ok, err = os.rename(full_old, full_new)
    if not ok then
...
local function delete_current()
  ...
  local entry = vim.api.nvim_buf_get_lines(state.buf, lnum-1, lnum, false)[1]
  if not entry or entry == "../" then
    vim.notify("Cannot delete this entry", vim.log.levels.WARN)
    return
  end
  ...
  local stat = stat_path(full)
  if not stat then
    vim.notify("File vanished", vim.log.levels.WARN)
    refresh()
    return
  end
  ...
  vim.ui.select({ "No", "Yes" }, {
    ...
    local ok, err
    if stat.type == "directory" then
      ok, err = pcall(vim.fn.delete, full, "rf")
    else
      ok, err = pcall(vim.fn.delete, full)
    end
    ...
    vim.notify("Deleted: " .. entry, vim.log.levels.INFO)
    refresh()
  end)
end
...
function M.setup(user_cfg)
  ...
  vim.keymap.set("n", M.config.mappings.rename, rename_current, buf_opts)
  vim.keymap.set("n", M.config.mappings.delete, delete_current, buf_opts)
